...
static int get_ept_root(struct kvm *kvm, uint64_t **rootp) {
    ...
    for (i = 0; i < vcpu_count; i++) {
        ...
        if (!vcpu)
            ERROR2(-ENXIO, "vcpu[%d] of process (pid = %d) is uncreated", i, kvm->userspace_pid);
        if (!(root_of_vcpu = vcpu->arch.mmu->root.hpa))
            ERROR1(-EINVAL, "vcpu[%d] is uninitialized", i);
        if (!root)
            root = root_of_vcpu;
        else if (root != root_of_vcpu)
            ERROR2(-EFAULT, "ept root of vcpu[%d] is %llx, different from other vcpus'", i, root_of_vcpu);
    }
...
#define EPT_VIOLATION_ACC_ALL   (EPT_VIOLATION_ACC_READ | EPT_VIOLATION_ACC_WRITE | EPT_VIOLATION_ACC_INSTR)
...
int sampler_init(struct sampler *sampler, pid_t pid, void (*func_on_sample)(unsigned long gpa, int xwr, void *privdata),
    ...
    if (!__sync_bool_compare_and_swap(&(kvm->ept_sample_handler), NULL, ept_sample_handler)) {
        kvm_put_kvm(kvm);
...
void sampler_deinit(struct sampler *sampler) {
    ...
    kvm = sampler->kvm;
    assert(kvm);
    assert(kvm->ept_sample_handler == ept_sample_handler);
    assert(kvm->ept_sample_privdata == sampler);
    kvm->ept_sample_privdata = NULL;
    wmb();
    kvm->ept_sample_handler = NULL;
    del_timer_sync(&(sampler->timer));
    EPT_WALK_TO_PMD(sampler->ept_root, ({
                        uint64_t pmd_val = (*pmdp);
                        if (pmd_val)
